...
/**
 * 首帧主题标记脚本。dsh-ux 配色是一层可选叠加（不动官方主题偏好，因为官方 settings 的
 * ui-theme 段只接受 light/dark/system，第三方主题存不进去且会被 adopt 覆盖），
 * 开关状态记在一枚 cookie 里。这行脚本由 host 注入到 body 起始处，在任何渲染前
 * 把开关落到 html 属性上，CSS 闸门据此生效 —— 否则会先按官方配色画一帧再跳成 Solarized。
 * cookie 缺失按「开」处理：与改造前（无条件接管浅色）的观感一致，老用户升级无感；
 * 深色模式另有 body:not([data-ds-dark-theme]) 拦着，不受影响。
 */
function themeMarkInjection() {
  return {
    kind: 'script',
    placement: 'body',
    text: `(()=>{try{var m=/(?:^|; )dsh-enhance-theme=([^;]*)/.exec(document.cookie);document.documentElement.setAttribute('data-dsh-ux-theme',m!==null&&m[1]==='off'?'off':'on')}catch(_){}})()`,
  }
}
