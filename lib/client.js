  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      * {
      ...
      /* ---- 工具调用折叠条 ---- */
      .dsh-round{display:flex;flex-wrap:wrap;align-items:center;gap:6px;min-width:0;max-width:100%}
      .dsh-tool-strip{display:flex;flex-wrap:nowrap;align-items:center;gap:2px 4px;width:fit-content;max-width:100%;min-width:0;padding:2px 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;background:var(--dsw-alias-interactive-bg-hover);cursor:pointer;user-select:none;font-size:12px;line-height:20px;color:var(--dsw-alias-label-secondary);overflow:hidden}
    ...
    function apply(ctx) {
      ...
      const scanFlow = (flow, force) => {
        ...
        for (const round of flow.querySelectorAll(":scope > .dsh-round")) {
          ...
          if (!isMidItem(next) && !isRound(next)) round.remove();
        }
        let group = [];
