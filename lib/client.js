window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SOLARIZED_LIGHT = {
      ...
      // 0.1.5 新增/改写的「凸起面」与代码块面:官方亮色一律是冷白(f5f6f7/fff/e9ecf2),
      // 落在我们的暖色底上就是"白框"(主题选中框、语言/权限/字号选择器、卡片激活态共 18 处)。
      // 按插件既有色阶换算:bg-layer-2(#e6dfc8,弹窗底) -> bg-layer-1(#eee8d5,凸起面) -> base3(#fdf6e3,选中/高亮)。
      "--dsw-alias-bg-module-platform": "#eee8d5",
      "--dsw-alias-bg-multi-select": "#eee8d5",
      "--dsw-alias-markdown-code-segment-selected": "#fdf6e3",
      "--dsw-alias-markdown-placeholder": "#e6dfc8",
      "--dsw-alias-markdown-tag": "#e6dfc8",
      "--dsw-alias-button-ghost-active-hover": "#e6dfc8",
      // present/交付卡片的文件行不吃 alias 层:它用包内变量 --deliverable-fill: var(--dsw-static-neutral-50)，
      // 亮色是冷白 #fafafa，在奶油底上就是一块"留白"。该静态 token 全官方只有 2 个消费者
      // (本卡片 + 我们已覆盖的 markdown 行内代码)，-100 只有 1 个(卡片 hover)，改它精准且不依赖哈希类名。
      "--dsw-static-neutral-50": "#eee8d5",
      "--dsw-static-neutral-100": "#e6dfc8",
    };
    const DARK_PRESERVE = {
      ...
      // 与上面同名的 6 个 token:值一律取官方深色原值,深色模式分毫不动。
      ...
      // 这两个静态 token 官方深浅取值相同，深色对填原值 => 深色模式构造上零差异
      // (深色下该卡片另走 --dsw-static-neutral-850/-800，不受影响)。
    ...
    function apply(ctx) {
      ...
      // theme 服务由 @deepseek-ai/dsh-client-ui-theme 提供,而 ui-theme 的 fiber 注入
      // ["slots","locale","remote","settingsScope"],要等宿主连接(remote)与设置作用域就绪后才
      // provide("theme")。本插件不声明 inject,apply 在启动瞬间就跑完,一次性 ctx.get("theme")
      // 只会读到 undefined —— 覆盖层被静默跳过,浅色底色回落到官方纯白(0.1.5 起官方把 light 的
      // bg-base / bg-layer-1..3 全部拍平为 #fff,所以缺了覆盖层就只剩白底)。
      // 改为响应式等待:theme 一出现就注册覆盖层,并把它挂在 fiber 上随卸载一起撤销。
      const applyThemeOverride = (theme) => {
        ...
        return theme.overrideTokens("solarized-light", overrides);
      };
      const attachTheme = (scope) => {
        const theme = scope.get("theme");
        if (theme === undefined) return;
        scope.effect(() => {
          try {
            return applyThemeOverride(theme);
          } catch (error) {
            console.error("[dsh-enhance] theme override failed", error);
            return undefined;
          }
        }, "dsh-enhance: solarized-light token layer");
      };
      if (ctx.get("theme") !== undefined) attachTheme(ctx);
      else ctx.inject(["theme"], attachTheme);
