  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      * {
      ...
      /* dsh-ux 配色：Solarized 浅色叠加层。
         它不接管官方主题偏好 —— 官方 settings 的 ui-theme 段只接受 light/dark/system
         （第三方主题存不进去，且 ThemeRuntime.adopt 会把内存偏好覆盖回内置值），
         所以本层自己带开关：html 属性由 host 在 index 注入的脚本按 cookie 在首帧前定好，
         浅色下生效、深色与「官方配色」下完全不生效。
         选择器特异性(html:not(...) body:not(...))高于官方 body 与 body[data-ds-dark-theme]；
         保留 !important 是为了压过组件级更具体的选择器（与改造前行为一致）。 */
      html:not([data-dsh-ux-theme="off"]) body:not([data-ds-dark-theme]) {
        /* —— alias 层：基础面 / 文字 / 边框 —— */
        --dsw-alias-bg-base: #fdf6e3 !important;
        --dsw-alias-bg-layer-1: #eee8d5 !important;
        --dsw-alias-bg-layer-2: #e6dfc8 !important;
        --dsw-alias-bg-layer-3: #eee8d5 !important;
        --dsw-alias-bg-overlay: #eee8d5 !important;
        ...
        --dsw-alias-border-l1: #e4dcc4 !important;
        --dsw-alias-border-l2: #c9bfa4 !important;
        ...
        --dsw-alias-brand-primary: #268bd2 !important;
        ...
        --dsw-alias-state-error-primary: #dc322f !important;
        --dsw-alias-state-success-primary: #859900 !important;
        --dsw-alias-state-warn-primary: #b58900 !important;
        /* —— specific 层：菜单 / 输入 / 气泡 / 侧边栏 —— */
        --dsw-specific-sidebar-fill: #eee8d5 !important;
        ...
        --dsw-alias-markdown-placeholder: #e6dfc8 !important;
        --dsw-alias-markdown-tag: #e6dfc8 !important;
        /* —— 交互态 / 按钮 —— */
        ...
        --dsw-alias-button-ghost-active-hover: #e6dfc8 !important;
        /* —— 浮层 / 滚动条 —— */
        ...
        /* —— 静态层：present/交付卡片的文件行不吃 alias 层，它用包内变量
           --deliverable-fill: var(--dsw-static-neutral-50)（亮色是冷白 #fafafa，
           在奶油底上就是一块"留白"）。该静态 token 全官方只有 2 个消费者
           （本卡片 + 已覆盖的 markdown 行内代码），-100 只有 1 个（卡片 hover）。 */
        --dsw-static-neutral-50: #eee8d5 !important;
        --dsw-static-neutral-100: #e6dfc8 !important;
      }
      ...
      /* ---- 外观设置行（模仿官方 AppearanceRow 的 .group/.cubeRow/.themeCube 度量）---- */
      .dsh-theme-row{display:flex;flex-direction:column;gap:8px;padding:16px 0;border-bottom:.5px solid var(--dsw-alias-border-l2)}
      .dsh-theme-title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}
      .dsh-theme-cubes{display:flex;flex-wrap:wrap;align-items:stretch;gap:8px}
      .dsh-theme-cube{box-sizing:border-box;flex:180px;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:4px;padding:20px 32px;border:.5px solid var(--dsw-alias-border-l4);border-radius:20px;background:0 0;color:var(--dsw-alias-label-primary);font:inherit;font-size:14px;line-height:22px;cursor:pointer}
      .dsh-theme-cube:hover:not(.dsh-theme-cube-on){background:var(--dsw-alias-interactive-bg-hover)}
      .dsh-theme-cube-on{background:var(--dsw-alias-bg-module-platform);border-color:var(--dsw-static-neutral-bluish-400)}
      .dsh-theme-cube-hint{font-size:12px;line-height:18px;color:var(--dsw-alias-label-tertiary)}
    ...
    // dsh-ux 配色的具体色值集中在顶部 CSS 的闸门块里
    // （html:not([data-dsh-ux-theme="off"]) body:not([data-ds-dark-theme])）。
    // 为什么不像官方主题那样 register 进主题注册表：官方 settings 的 ui-theme 段只接受
    // light/dark/system，第三方主题的偏好存不进去，且 ThemeRuntime.adopt 会把内存偏好
    // 覆盖回内置值（实测验证）；所以改成一枚 cookie + html 属性做开关，
    // 读写见 apply 里的 themeEnabled / setThemeEnabled。
    ...
    // 外观设置行。官方那三个方块（浅色/深色/跟随系统）是硬编码的 CUBES 数组，
    // 第三方主题进不去，所以同区自建一行承载「dsh-ux 配色」开关（order 20，排在官方外观行之后）。
    // 职责划分：官方行管明暗偏好，本行只管是否在浅色之上叠加 Solarized 配色。
    // 状态不存官方主题偏好（那个段只接受 light/dark/system，会被 adopt 覆盖），
    // 而是读写 html 属性 + cookie（cookie 供 host 在首帧前恢复开关）。
    function ThemeRow({ isEnabled, setEnabled }) {
      const [on, setOn] = React.useState(() => isEnabled());
      const cube = (key, label, hint, selected, onClick) => React.createElement("button", {
        key: key,
        type: "button",
        className: "dsh-theme-cube" + (selected ? " dsh-theme-cube-on" : ""),
        "aria-pressed": selected,
        onClick: onClick,
      },
        React.createElement("span", null, label),
        React.createElement("span", { className: "dsh-theme-cube-hint" }, hint));
      const choose = (next) => { setEnabled(next); setOn(next); };
      return React.createElement("div", { className: "dsh-theme-row" },
        React.createElement("div", { className: "dsh-theme-title" }, "dsh-ux 配色"),
        React.createElement("div", { className: "dsh-theme-cubes" },
          cube("ux", "Solarized 暖色", "浅色下叠加插件配色", on, () => choose(true)),
          cube("official", "官方配色", "回到 DeepSeek 默认", !on, () => choose(false))));
    }
    ...
    function apply(ctx) {
      ...
      // 外观设置行：官方那三个方块(浅色/深色/跟随系统)是硬编码的 CUBES 数组，第三方主题
      // 进不去，所以同区自建一行(order 20，排在官方外观行之后)承载「dsh-ux 配色」开关。
      // 开关状态不走官方主题偏好：官方 settings 的 ui-theme 段只接受 light/dark/system
      // (第三方主题存不进去，且 ThemeRuntime.adopt 会把内存偏好覆盖回内置值 —— 实测验证过)，
      // 所以用一枚 cookie 记状态：host 侧在 index 注入的脚本读它、在任何渲染前把
      // data-dsh-ux-theme 写到 html 上，CSS 闸门据此决定这层配色是否生效。
      const THEME_ROW_ID = "dsh-enhance-theme";
      const THEME_COOKIE = "dsh-enhance-theme";
      const themeEnabled = () => {
        try {
          return document.documentElement.getAttribute("data-dsh-ux-theme") !== "off";
        } catch (_) {
          return true;
        }
      };
      const setThemeEnabled = (on) => {
        const value = on ? "on" : "off";
        document.documentElement.setAttribute("data-dsh-ux-theme", value);
      ...
      slots.inject("settings.general.item", () => slots.register(
        { name: "settings.general.item", id: THEME_ROW_ID, order: 20 },
        () => React.createElement(ThemeRow, { isEnabled: themeEnabled, setEnabled: setThemeEnabled }),
      ));
